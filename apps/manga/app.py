...
def test_upload():
    print(f"DEBUG: Test upload route called with method: {request.method}")
    return jsonify({'message': 'Test upload endpoint working', 'method': request.method})

@app.route('/upload', methods=['POST'])
def upload_files():
    import sys
    print("🔥 UPLOAD ROUTE CALLED!", file=sys.stderr, flush=True)
    ...
    try:
        ...
        if len(files) > 500:
            return jsonify({'error': 'Too many files. Maximum 500 files allowed.'}), 400
...
def create_automated_video():
    ...
    try:
        ...
        response_data = {
            ...
            'tts_method': result.get('tts_method', 'Unknown'),
            ...
            try:
                ...
                for i, segment in enumerate(text_segments):
                    try:
                        ...
                        print(f"🎤 Generating TTS for segment {i}: '{segment[:50]}...'")
                        
                        # Try Windows Edge TTS first (most reliable)
                        tts_success = False
                        try:
                            voice = "en-US-JennyNeural"  # High-quality Windows voice
                            ...
                            if os.path.exists(audio_path) and os.path.getsize(audio_path) > 0:
                                print(f"✅ Windows TTS success for segment {i}: {os.path.getsize(audio_path)} bytes")
                                audio_files.append(audio_path)
                                tts_success = True
                            else:
                                print(f"❌ Windows TTS failed for segment {i}: file not created or empty")
                        except Exception as e:
                            print(f"❌ Windows TTS exception for segment {i}: {e}")
                        
                        # Fallback to OpenAI TTS if Windows TTS failed and we have API key
                        if not tts_success and openai_api_key:
                            try:
                                print(f"🔄 Trying OpenAI TTS fallback for segment {i}")
                                openai_client = OpenAI(api_key=openai_api_key)
                                
                                response = openai_client.audio.speech.create(
                                    model="tts-1",
                                    voice="alloy",
                                    input=segment[:4000]  # OpenAI has character limits
                                )
                                ...
                                if os.path.exists(audio_path) and os.path.getsize(audio_path) > 0:
                                    print(f"✅ OpenAI TTS success for segment {i}: {os.path.getsize(audio_path)} bytes")
                                    audio_files.append(audio_path)
                                    tts_success = True
                                else:
                                    print(f"❌ OpenAI TTS failed for segment {i}: file not created")
                            except Exception as e:
                                print(f"❌ OpenAI TTS exception for segment {i}: {e}")
                        
                        # If both TTS methods failed, append None but continue
                        if not tts_success:
                            print(f"⚠️ All TTS methods failed for segment {i}, video will have no audio for this segment")
                            audio_files.append(None)
                            
                    except Exception as e:
                        print(f"❌ Overall TTS failed for segment {i}: {e}")
                        audio_files.append(None)
                
                yield f"data: {json.dumps({'progress': 65, 'message': 'Creating optimized video with enhanced image processing...', 'step': 3})}\n\n"
                
                # Debug: Check audio generation results
                successful_audio = [af for af in audio_files if af is not None]
                print(f"🎵 Audio generation summary: {len(successful_audio)}/{len(audio_files)} segments have audio")
                for i, af in enumerate(audio_files):
                    if af:
                        size = os.path.getsize(af) if os.path.exists(af) else 0
                        print(f"   Segment {i}: {af} ({size} bytes)")
                    else:
                        print(f"   Segment {i}: NO AUDIO")
                ...
                try:
                    ...
                    for i, (segment, image_file) in enumerate(zip(text_segments, matched_images)):
                        ...
                        try:
                            ...
                            # Explicitly load and attach audio with verification
                            if i < len(audio_files) and audio_files[i] and os.path.exists(audio_files[i]):
                                try:
                                    audio_clip = AudioFileClip(audio_files[i])
                                    print(f"🔊 Loading audio for segment {i}: {audio_files[i]} (duration: {audio_clip.duration:.2f}s)")
                                    img_clip = img_clip.set_audio(audio_clip)
                                    print(f"✅ Audio attached successfully to segment {i}")
                                except Exception as e:
                                    print(f"❌ Failed to attach audio for segment {i}: {e}")
                                    # Continue without audio for this segment
                            else:
                                print(f"⚠️ No audio file for segment {i} - segment will be silent")
                                print(f"   Audio files list length: {len(audio_files)}")
                                if i < len(audio_files):
                                    print(f"   Audio file for segment {i}: {audio_files[i]}")
                                    if audio_files[i]:
                                        print(f"   File exists: {os.path.exists(audio_files[i])}")
                            ...
                            # Explicitly load and attach audio with verification (fallback)
                            if i < len(audio_files) and audio_files[i] and os.path.exists(audio_files[i]):
                                try:
                                    audio_clip = AudioFileClip(audio_files[i])
                                    print(f"🔊 Loading audio for segment {i} (fallback): {audio_files[i]} (duration: {audio_clip.duration:.2f}s)")
                                    img_clip = img_clip.set_audio(audio_clip)
                                    print(f"✅ Audio attached successfully to segment {i} (fallback)")
                                except Exception as e:
                                    print(f"❌ Failed to attach audio for segment {i} (fallback): {e}")
                                    # Continue without audio for this segment
                            else:
                                print(f"⚠️ No audio file for segment {i} - segment will be silent (fallback)")
                    ...
                    video_url = f"/manga/uploads/{session_id}/{output_filename}"
...
@app.route('/test-moviepy', methods=['GET'])
def test_moviepy():
    """Test endpoint to check if MoviePy is working"""
    try:
        from moviepy.editor import ImageClip, concatenate_videoclips
        
        # Create a simple test - just import and create basic clip
        test_info = {
            'moviepy_imported': True,
            'test_status': 'MoviePy basic imports successful'
        }
        
        # Try creating a simple colored clip
        try:
            from moviepy.editor import ColorClip
            test_clip = ColorClip(size=(640,480), color=(255,0,0), duration=1)
            test_info['color_clip'] = 'Success'
            test_clip.close()  # Clean up
        except Exception as e:
            test_info['color_clip'] = f'Failed: {str(e)}'
...
def test_ffmpeg():
    """Test FFmpeg codec availability and path handling"""
    try:
        ...
        test_filenames = [
            "manga_video_20250827:161119.mp4",  # Problematic colon
            "manga video with spaces.mp4",
            "manga-video-with-dashes.mp4",
            "normal_filename.mp4"
        ]
        
        sanitized = {}
        for filename in test_filenames:
            sanitized[filename] = sanitize_filename(filename)
        
        # Test temp path creation
        temp_path = get_safe_temp_path("test_audio")
        
        return jsonify({
            'ffmpeg_status': 'available' if codecs['libx264'] or codecs['h264'] else 'missing_codecs',
            'available_codecs': codecs,
            'filename_sanitization': sanitized,
            'temp_path_example': temp_path,
            'system_info': {
...
if __name__ == '__main__':
    try:
        ...
        print(f"Claude API key configured: {'Yes' if app.config['CLAUDE_API_KEY'] else 'No'}")
        print(f"OpenAI API key configured: {'Yes' if app.config['OPENAI_API_KEY'] else 'No'}")
